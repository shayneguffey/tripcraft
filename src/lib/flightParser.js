...
const AIRPORT_CITIES = {
  LAX: "Los Angeles", ICN: "Seoul-Incheon", NRT: "Tokyo-Narita", HND: "Tokyo-Haneda",
  TYO: "Tokyo", BKK: "Bangkok", SFO: "San Francisco", SEA: "Seattle", JFK: "New York",
  ORD: "Chicago", ATL: "Atlanta", DFW: "Dallas", DEN: "Denver", SLC: "Salt Lake City",
  PDX: "Portland", PHX: "Phoenix", LAS: "Las Vegas", SAN: "San Diego", IAH: "Houston",
  MIA: "Miami", BOS: "Boston", EWR: "Newark", IAD: "Washington DC", CLT: "Charlotte",
  MSP: "Minneapolis", DTW: "Detroit", PHL: "Philadelphia", MCO: "Orlando", TPA: "Tampa",
  LHR: "London", CDG: "Paris", FRA: "Frankfurt", AMS: "Amsterdam", FCO: "Rome",
  BCN: "Barcelona", MAD: "Madrid", IST: "Istanbul", DXB: "Dubai", SIN: "Singapore",
  HKG: "Hong Kong", TPE: "Taipei", PEK: "Beijing", PVG: "Shanghai", KIX: "Osaka",
  CTS: "Sapporo", CNX: "Chiang Mai", HNL: "Honolulu", OGG: "Maui", GUM: "Guam",
  SYD: "Sydney", MEL: "Melbourne", AKL: "Auckland", YVR: "Vancouver", YYZ: "Toronto",
  MEX: "Mexico City", CUN: "Cancun", LIM: "Lima", GRU: "São Paulo", EZE: "Buenos Aires",
  BOG: "Bogotá", SCL: "Santiago", MNL: "Manila", SGN: "Ho Chi Minh", HAN: "Hanoi",
  KUL: "Kuala Lumpur", CGK: "Jakarta", DEL: "New Delhi", BOM: "Mumbai",
};

const AIRLINE_NAMES = {
  DL: "Delta", AA: "American", UA: "United", WN: "Southwest", AS: "Alaska",
  B6: "JetBlue", NK: "Spirit", F9: "Frontier", HA: "Hawaiian", KE: "Korean Air",
  OZ: "Asiana", NH: "ANA", JL: "JAL", CX: "Cathay Pacific", SQ: "Singapore Airlines",
  TG: "Thai Airways", BR: "EVA Air", CI: "China Airlines", MH: "Malaysia Airlines",
  QF: "Qantas", NZ: "Air New Zealand", AC: "Air Canada", BA: "British Airways",
  LH: "Lufthansa", AF: "Air France", KL: "KLM", IB: "Iberia", TK: "Turkish Airlines",
  EK: "Emirates", QR: "Qatar Airways", EY: "Etihad", CZ: "China Southern",
  CA: "Air China", MU: "China Eastern", PR: "Philippine Airlines", VN: "Vietnam Airlines",
};
...
// Detect what kind of input the user pasted and parse accordingly
export function parseFlightInput(input) {
  ...
  if (trimmed.startsWith("http://") || trimmed.startsWith("https://")) {
    if (trimmed.includes("google.com/travel/flights")) {
      return { type: "google_flights", url: trimmed, legs: parseGoogleFlightsUrl(trimmed) };
    }
    if (trimmed.includes("skyscanner.com")) {
      return { type: "skyscanner", url: trimmed, legs: parseSkyscannerUrl(trimmed) };
    }
    if (trimmed.includes("delta.com")) {
      return { type: "delta", url: trimmed, legs: parseDeltaUrl(trimmed) };
    }
    if (trimmed.includes("united.com")) {
      return { type: "united", url: trimmed, legs: parseUnitedUrl(trimmed) };
    }
    // Generic URL — can't parse, but store it
    return { type: "unknown_url", url: trimmed, legs: [] };
  }

  // Otherwise try to parse as text
  const legs = parseFlightText(trimmed);
  return { type: "text", legs };
}
...
function parseGoogleFlightsUrl(url) {
  ...
  try {
    ...
    if (unique.length >= 2) {
      legs.push({ direction: "outbound", departure_airport: unique[0], arrival_airport: unique[1], leg_order: 0 });
      legs.push({ direction: "return", departure_airport: unique[1], arrival_airport: unique[0], leg_order: 1 });
    }
  } catch (e) {
  ...
  try {
    ...
    for (let i = 0; i < raw.length - 2; i++) {
      ...
      if (allPrintable && str.length >= 2) {
        ...
        protoStrings.push({ field: fieldNum, text: str, pos: i });
      }
    }
...
function tryExtractPriceFromTfu(tfu) {
  try {
    ...
    for (let i = 0; i < raw.length - 2; i++) {
      ...
      if (str.match(/^[A-Z]{3}$/) && ["USD", "EUR", "GBP", "JPY", "KRW", "CAD", "AUD", "CNY", "THB", "SGD", "MXN"].includes(str)) {
        currency = str;
        currencyPos = i;
        break;
      }
    ...
    if (!currency) {
      ...
      for (let i = 0; i < raw.length - 2; i++) {
        ...
        for (let j = 0; j < len; j++) {
          const c = raw.charCodeAt(i + 2 + j);
          if (c < 32 || c > 126) { isB64 = false; break; }
          str += String.fromCharCode(c);
        }
        ...
        try {
          ...
          for (let k = 0; k < innerRaw.length - 2; k++) {
            ...
            if (ilen === 3 && k + 5 <= innerRaw.length) {
              ...
              if (istr.match(/^[A-Z]{3}$/) && ["USD", "EUR", "GBP", "JPY", "KRW", "CAD", "AUD"].includes(istr)) {
                currency = istr;
...
// Parse raw flight text (pasted from Google Flights, emails, etc.)
    ...
    for (const line of lines) {
      ...
      if (tMatch) {
        if (pending.departure_airport) {
          legs.push({ ...pending, direction: order === 0 ? "outbound" : "return", leg_order: order++ });
        }
        pending = { departure_time: convertTo24h(tMatch[1]), arrival_time: convertTo24h(tMatch[2]), departure_date: currentDate };
        continue;
      }
...
// Smart OCR text parser — extracts flight data from messy Tesseract output
// Handles airline booking pages (Alaska, Delta, United, AA, Google Flights, etc.)
export function parseOcrText(rawText) {
  if (!rawText || rawText.trim().length < 10) return { legs: [], price: null, currency: null };
  ...
  for (const pat of pricePatterns) {
    ...
    if (m) {
      ...
      if (val >= 20 && val <= 50000) { price = val; break; }
    }
  }
  ...
  while ((pm = parensPattern.exec(fullText)) !== null) {
    if (isKnownAirport(pm[1])) {
      airportInstances.push({ code: pm[1], index: pm.index, fromParens: true });
    }
  }
  // Also look for standalone codes not already found
  const standalonePattern = /\b([A-Z]{3})\b/g;
  const NOISE_WORDS = new Set(["THE","AND","FOR","ARE","BUT","NOT","YOU","ALL","CAN","HER","WAS","ONE","OUR","OUT","DAY","HAD","HAS","HIS","HOW","ITS","MAY","NEW","NOW","OLD","SEE","WAY","WHO","DID","GET","HIM","LET","SAY","SHE","TOO","USE","MOM","DAD","SON","SIT","TRY","ASK","MEN","RUN","BIG","FEW","TAX","NET","TIP","FEE","PER","VIA","EST","PDT","PST","CST","MST","EDT","CDT","MDT","GMT","UTC","MON","TUE","WED","THU","FRI","SAT","SUN","JAN","FEB","MAR","APR","JUN","JUL","AUG","SEP","OCT","NOV","DEC","USD","EUR","GBP","CAD","AUD","JPY","KRW","THB","SGD","MXN","CNY","MIN","HRS","AVG","MAX","ADD","FLY","AIR","JET"]);
  while ((pm = standalonePattern.exec(fullText)) !== null) {
    ...
    if (isKnownAirport(pm[1]) && !airportInstances.find(a => a.code === pm[1] && Math.abs(a.index - pm.index) < 5)) {
      airportInstances.push({ code: pm[1], index: pm.index, fromParens: false });
    }
  }
  ...
  while ((routeMatch = cityRoutePattern.exec(fullText)) !== null) {
    routeInstances.push({ from: routeMatch[2], to: routeMatch[4], index: routeMatch.index });
  }
  ...
  if (routeInstances.length > 0) {
    ...
    for (let i = 0; i < routeInstances.length; i++) {
      ...
      const nearDir = directionMarkers.filter(d => d.index < sectionStart && d.index >= sectionStart - 100)
...
function isKnownAirport(code) {
  ...
  const MORE_AIRPORTS = ["ANC","ABQ","AUS","BNA","BDL","BUF","BWI","CHS","CLE","CMH","CVG","DAL","DSM","ELP","FLL","GRR","GSO","GSP","HOU","IND","JAX","MCI","MEM","MKE","MSY","OAK","OKC","OMA","ONT","PBI","PIT","PVD","RDU","RIC","RNO","RSW","SAT","SDF","SJC","SJU","SMF","SNA","STL","TUL","TUS"];
