 ...
 *
 * Collapsible: collapsed by default. Header bar is the toggle. Body shows
 * the textarea / preview only when expanded.
 *
  ...
  return (
    <div className="px-5 py-4 border-t border-stone-200/60">
      <button
        ...
        title={expanded ? "Collapse journal" : "Expand to write a journal entry"}
      ...
      >
        ...
        {hasEntry && !expanded && (
          <span className="text-[10px] text-emerald-600 italic ml-2">Entry saved</span>
        )}
        <svg
          className={`w-3.5 h-3.5 ml-auto text-stone-400 transition-transform ${expanded ? "rotate-180" : ""}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
          strokeWidth={2}
      ...
      {expanded && (
        <div className="mt-2">
          {savedAt && content && !editing && (
            <p className="text-[10px] text-stone-400 italic mb-1">{formatSavedAt(savedAt)}</p>
          )}
          ...
          {saving && <p className="text-[10px] text-stone-400 mt-1 italic">{"Saving…"}</p>}
        </div>
      )}
    </div>
  );
