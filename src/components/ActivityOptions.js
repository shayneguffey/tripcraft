...
// MAIN COMPONENT
// ─────────────────────────────────────────────────
export default function ActivityOptions({ tripId, tripStart, tripEnd, onActivityOptionsChange }) {
  ...
  return (
    <div className="mb-6">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={() => setShowModal(true)} className="px-4 py-2 bg-emerald-600 text-white text-sm font-semibold rounded-lg hover:bg-emerald-700 transition-colors">
          + Add Activity Option
        </button>
      </div>
        ...
        <div className="flex gap-4">
          ...
          <div className="w-56 flex-shrink-0 space-y-2">
            ...
            <button onClick={() => setShowModal(true)}
              className="w-full py-3 border-2 border-dashed border-emerald-300 rounded-xl text-emerald-600 text-sm font-medium hover:bg-emerald-50 transition-colors">
              + Add Option
            </button>
          </div>
      ...
      {options.length === 0 && !showModal && (
        <div className="bg-white rounded-xl border border-sky-100 shadow-sm p-8 text-center">
          <svg className="w-10 h-10 text-emerald-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
          </svg>
          ...
          <button onClick={() => setShowModal(true)} className="px-4 py-2 bg-emerald-600 text-white text-sm font-semibold rounded-lg hover:bg-emerald-700">
            + Add Activity Option
          </button>
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mt-1">
        ...
        {opt.price && <span className="text-sm font-bold text-emerald-600">{formatPrice(opt.price, opt.currency)}</span>}
      </div>
    </div>
  );
}
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={onToggleSelected}
          className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
            opt.is_selected ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600 hover:bg-emerald-50"
          }`}>
...
function AddActivityModal({ tripId, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl border border-emerald-100 w-full max-w-lg max-h-[85vh] overflow-y-auto"
        ...
        <div className="p-6">
            ...
            {!screenshot ? (
              <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
                ...
                className={`border-2 border-dashed rounded-lg p-6 text-center transition-colors cursor-pointer ${
                  dragOver ? "border-emerald-400 bg-emerald-50" : "border-slate-300 hover:border-emerald-400"
                }`}
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleSave} disabled={saving || !name.trim()}
              className="flex-1 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-semibold hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed">
