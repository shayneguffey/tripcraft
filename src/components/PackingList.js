...
export default function PackingList({ tripId, tripDestination, tripStartDate, tripEndDate, activityOptions, accommodationOptions }) {
  ...
  async function handleSaveTemplate(e) {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser();
      const templateItems = items.map((i) => ({ text: i.text, category: i.category }));
      await supabase.from("saved_packing_templates").insert({
        user_id: user.id,
  ...
  async function deleteSavedTemplate(templateId) {
    await supabase.from("saved_packing_templates").delete().eq("id", templateId);
      ...
      <div className="bg-white rounded-xl border border-indigo-100 shadow-sm overflow-hidden">
        ...
        {showSaveDialog && (
          <div className="px-4 py-3 border-b border-slate-100 bg-amber-50/50">
            <p className="text-xs text-slate-500 mb-2">Save your current packing list as a reusable template:</p>
            <form onSubmit={handleSaveTemplate} className="flex items-center gap-2">
              <input
                type="text"
                value={templateName}
        ...
        {showTemplates && (
          <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/50">
            ...
            {savedTemplates.length > 0 && (
              <>
                <p className="text-xs text-slate-500 mt-3 mb-2">Your saved templates:</p>
                <div className="flex flex-wrap gap-2">
                  {savedTemplates.map((tmpl) => (
                    <div key={tmpl.id} className="inline-flex items-center rounded-lg border border-amber-200 bg-white overflow-hidden">
                      <button
                      ...
                      >
                        ...
                        {tmpl.name}
                        <span className="text-[10px] text-slate-400">({(tmpl.items || []).length})</span>
                      </button>
                      <button
