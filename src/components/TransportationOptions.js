...
export default function TransportationOptions({ tripId, tripStart, tripEnd, onTransportationOptionsChange, itinerarySelections, activeItineraryId, onToggleSelection }) {
  ...
  async function handleToggleBooked(id) {
    ...
    await supabase.from("transportation_options").update({ booked: newBooked }).eq("id", id);
    ...
    if (newBooked && onToggleSelection && activeItineraryId) {
      const isInItinerary = (itinerarySelections || []).some(
        (s) => s.option_type === "transportation" && s.option_id === id
      );
      if (!isInItinerary) onToggleSelection("transportation", id);
    }
    loadOptions();
  }
...
// ─── OPTION DETAIL ───
function OptionDetail({ opt, tripStart, tripEnd, onToggleSelected, onToggleBooked, onTimeChange, onNotesChange }) {
  const cat = getCategoryInfo(opt.category);
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-violet-100 w-full max-w-lg max-h-[85vh] overflow-y-auto"
        ...
        <div className="p-6">
          ...
          <div className="mb-4">
            ...
            <p className="text-xs text-slate-400 mb-2">Paste a rental car, train, ferry, or transfer URL. Or paste / drop a screenshot.</p>
            {!screenshot ? (
              <div
                ...
                className={`border-2 border-dashed rounded-lg transition-colors ${
                  dragOver ? "border-purple-400 bg-purple-50" : "border-slate-300"
                }`}
              >
                <textarea
                  value={pasteInput}
                  ...
                  rows={2}
                  autoFocus
                  placeholder={`https://www.hertz.com/... — or paste an image`}
            ...
            {urlStatus === "done" && aiResult?.name && (
              <div className="mt-2 px-3 py-2 bg-purple-50 rounded-lg text-xs text-purple-700">
                Found: {aiResult.name}
              </div>
            )}
